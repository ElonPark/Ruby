...
# 변수는 객체인가? 루비에서 아니다. 변수는 단순히 객체에 대한 참조를 가지고 있을 뿐이다.
# 객체는 커다란 풀 어딘가를(대부분의 경우 힙) 떠다니고 있을 뿐이고 변수가 그것을 가리키고 있다.
...
# person1을 person2에 대입해도 새로운 객체는 생성되지 않는다.
# 단지 person1 객체에 대한 참조를 person2로 복사해서 person1과 person2가 같은 객체를 참조하도록 만들 뿐이다.

# 대입은 객체의 별명(alias)을 늘려서 결과적으로 여러 개의 변수가 하나의 객체를 참조하로도록 한다.
# 그런데 이러한 점이 코드에서 문제를 일으킬 수 있지 않을까?
# 물론 그럴 수도 있지만, 우리가 생각하는 것 만큼 자주는 아니다(자바 객체로 같은 방식으로 동작한다).
